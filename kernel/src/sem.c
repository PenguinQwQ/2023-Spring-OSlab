...
void sem_p(sem_t *sem) {
  ...
  // TODO();
  sem->value--;
  if(sem->value < 0) //There is no source!
    {
...
void sem_v(sem_t *sem) {
  ...
  if(sem->value <= 0)
  {
    void *ptr = list_dequeue(&(sem->wait_list));//Get a free proc from the proc list!
    proc_addready((proc_t *)(ptr));
  }
}
...
usem_t *usem_alloc(int value) {
  ...
  // TODO();
  for (int i = 0 ; i <= USER_SEM_NUM - 1 ; i++)
    {
      if(user_sem[i].ref == 0) //meaning it is empty!
        {
